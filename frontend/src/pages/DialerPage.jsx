...
function DialerPage() {
  ...
  return (
    <div className="flex h-full gap-0 -m-6">
      ...
      {/* MIDDLE Panel - Context / Idle Greeting (unaffected by call state) */}
      ...
      {/* RIGHT Panel - Dialer / Active Call (fixed width, content swaps internally) */}
      <div className="w-72 bg-gray-800 border-l border-gray-700 p-5 flex flex-col items-center justify-center">
        {!isCallActive ? (
          <>
            {/* Number Display */}
            <div className="w-full mb-8">
              <div className="bg-gray-700 border border-gray-600 rounded-xl px-4 py-3 text-center min-h-[56px] flex items-center justify-center">
                {number ? (
                  <span className="text-2xl font-mono text-white tracking-wider">{number}</span>
                ) : (
                  <span className="text-gray-500 text-base">Enter number</span>
                )}
              </div>
            </div>
                ...
                <div key={i} className="flex gap-2">
                  {row.map((key) => (
                    <button
                      ...
                      className="flex-1 bg-gray-700 hover:bg-gray-600 active:bg-gray-500 border border-gray-600 rounded-xl py-4 text-xl font-medium text-white transition-colors cursor-pointer"
                    >
                      {key}
                    </button>
                  ))}
            ...
            <div className="w-full mt-6">
              <button
                ...
                className="w-full bg-green-500 hover:bg-green-600 disabled:bg-gray-700 disabled:text-gray-500 py-3 rounded-xl text-white font-semibold text-base transition-colors cursor-pointer flex items-center justify-center gap-2"
              >
                <Phone size={20} /> Call
              </button>
            </div>
          ...
          /* Active Call Screen — same fixed-width panel, content swapped */
          <div className="flex flex-col items-center justify-center space-y-6">
            <div className="text-center">
              <p className="text-gray-400 text-sm">Calling...</p>
              <p className="text-xl font-mono text-white mt-2 tracking-wider">{number}</p>
              <p className="text-green-400 text-base font-medium mt-3">00:15</p>
            </div>
            <div className="flex gap-3 flex-wrap justify-center">
              ...
              <button className="w-12 h-12 bg-gray-700 hover:bg-gray-600 rounded-full flex items-center justify-center text-gray-300 transition-colors cursor-pointer">
                <Pause size={20} />
              </button>
              <button onClick={handleHangup}
                className="w-12 h-12 bg-red-600 hover:bg-red-700 rounded-full flex items-center justify-center text-white transition-colors cursor-pointer">
                <PhoneOff size={20} />
              </button>
            </div>
          </div>

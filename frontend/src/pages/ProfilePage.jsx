...
import { ArrowLeft, User, Mail, Phone, FileText, Pencil, Check, X, AlertCircle, Camera, Globe, Users, EyeOff } from 'lucide-react';
...
function ProfilePage() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('user', JSON.stringify({ ...user, name: data.name, avatar, photoVisibility }));
  ...
  const handleCancel = () => {
    ...
    setPhotoVisibility(user.photoVisibility || 'everyone'); setEditing(false); setError(''); setMsg('');
  };
      ...
      {editing && (
        <form onSubmit={handleSave} className="bg-gray-800 rounded-xl border border-gray-700 p-6 space-y-5">
          ...
          <div>
            ...
            <input type="email" value={user.email} disabled className="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-4 py-2.5 text-sm text-gray-500 cursor-not-allowed" />
